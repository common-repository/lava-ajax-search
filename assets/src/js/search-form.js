	...
	lava_ajax_search.prototype.cache = function() {
		...
		return {
			...
			getResults : function( keyword, callback, failCallback ) {
				...
				inst = $.post( self.args.ajaxurl, postData,
					function( xhr ) {
	...
	lava_ajax_search.prototype.autocomplete.render = function() {
		...
		return function( ul, item ) {
			...
			if(oldType != item.type) {
				$("<li>").attr("class", 'type-' + oldType + " last-type invisible " + is_ajax ).appendTo(ul);
			}
			oldType = item.type;
			if (item.type_label != "") {
	...
	lava_ajax_search.prototype.setSelectize = function() {
		...
		if( 0 < element.length && typeof $.fn.autocomplete != 'undefined' ) {
			let autocompleteArgs = {
				...
				open: function() {
					let appendTo = $(this).autocomplete('option', 'appendTo');
					...
					if(appendTo) {
						$(appendTo).addClass('results-loaded');
					}
				},
			}
			if($( element.data('results-target') ).length) {
				autocompleteArgs.appendTo = element.data('results-target');
			}
			element.autocomplete(autocompleteArgs).on( 'focus', function() {
				$( this ).autocomplete( 'search', $( this ).val() );
			}).data("ui-autocomplete")._renderItem = self.autocomplete.render();
		}
	}

	lava_ajax_search.prototype.bindEvent = function() {
		let self = this;
		$('.actions', self.el).each(function(){
			let $action = this;
			$('.clear', $action).on('click', function(){
				$('input[data-search-input]', self.el).val('');
			})
		});
	}
