...
class Lava_Ajax_Search_Core {
	...
	public function register_hooks() {
		...
		add_filter( 'lava/ajax-search/type/label', Array( $this, 'labelFilter' ), 10, 2);
	}
	...
	public function labelFilter( $label='', $type='' ) {
	...
	public function searchResponse() {
		...
		if( isset( $this->search_results['all']['items'] ) && !empty( $this->search_results['all']['items'] ) ){
			...
			foreach( $this->search_results['all']['items'] as $item_id=>$item ){
				...
				$type_label = apply_filters( 'lava/ajax-search/type/label', $item['type'], $item['type']);
	...
	public function do_search( $args='' ){
		...
		$keyword->add_keyword($args['search_term']);
			...
			if( !empty( $results ) ){
				...
				if( !empty( $this->search_results['all']['items'] ) && $args['template_type']!='ajax' ){
					...
					foreach( $ordered_items_group as $type=>&$items ){
						...
						$label = apply_filters( 'lava/ajax-search/type/label', $label, $type);
						...
						$end_html .= sprintf(
							'<a href="%1$s" class="btn btn-sm btn-see-more btn-type-%2$s">' .
