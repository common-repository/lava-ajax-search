...
if( ! class_exists( 'Lava_Ajax_Search' ) ) :
	...
	class Lava_Ajax_Search {
		...
		const STACK_TABLE = 'keyword_stack';
		...
		public function register() {
			$strFile = ABSPATH . 'wp-admin/includes/upgrade.php';
			$charset_collate = $GLOBALS['wpdb']->get_charset_collate();

			$strQuery = "
			CREATE TABLE IF NOT EXISTS " . $GLOBALS['wpdb']->prefix . self::STACK_TABLE . "(
		...
		public function unregister(){
			if (defined('WP_UNINSTALL_PLUGIN')){
				$GLOBALS[ 'wpdb' ]->query( "DROP TABLE IF EXISTS " . $GLOBALS['wpdb']->prefix . self::STACK_TABLE);
			}
			do_action( $this->getHookName( 'Unregister' ) );
		}
		...
		public function load_files() {
			...
			require_once( 'includes/class-keyword.php' );
